...
# read in sensor data
source("./code/clean_sensor.R")
tl <- clean_sensor(path="/Volumes/Urdintxu/sensors_meso", n_sensor = 7)
tl_m <- subset(tl, select = c('Date', 'Time', 'date_time', 'TemperatureC'))
tl_m <- na.omit(tl_m)
tl_m$date_time <- strptime(tl_m$date_time, 
                           format = "%Y-%m-%d %H:%M:%S",
                           tz = "UTC")
tl_m$date_time <- as.POSIXct(tl_m$date_time)

# load weather station data
load(file='./data/weatherIVILAR12_hourly.RData')
# subset to dates recorded by the sensor
weather <- weather_h[weather_h$date >= (tl_m[1, 'date_time'] + 3600) & 
                       weather_h$date <= (tl_m[nrow(tl_m), 'date_time'] + 3600), ]
...
micro <- micro_custom(41.1073, -8.5897, 230, weather, 
                      th=F, habitat='herb', slope=0, azmuth=180)


metout <- as.data.frame(micro$metout) # above ground microclimatic conditions, min shade
soil <- as.data.frame(micro$soil) # soil temperatures, minimum shade

with(tl_m, plot(TemperatureC ~ date_time, type = 'l', col = '1', ylim = c(5, 35)))
with(metout, plot( ~ date, type = 'l', col = '1', ylim = c(5, 35)))
