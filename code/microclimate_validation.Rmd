...
tloc_sun_0cm <- data.frame(datetime = round(as.POSIXct(strptime(tloc_sun_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),temp = tloc_sun_0cm[,3])
year(tloc_sun_0cm$datetime) <- 2022

# soil sun
tsoil_sun_0cm <- read.csv(file = 'MS002T.csv', head=T)
tsoil_sun_0cm <- data.frame(datetime = round(as.POSIXct(strptime(tsoil_sun_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),temp = tsoil_sun_0cm[,3])
year(tsoil_sun_0cm$datetime) <- 2022

# soil sun buried (5 cm)
tsoil_sun_buried <- read.csv(file = 'MS005T.csv', head=T)
tsoil_sun_buried <- data.frame(datetime = round(as.POSIXct(strptime(tsoil_sun_buried[,1], "%d/%m/%Y %H:%M")),"hours"),temp = tsoil_sun_buried[,3])
...
with(soil, plot(micro$dates, D30cm, type='l', 
                ...
                xlab='', ylab='Tsoil sun 30 cm (ºC)'))
with(tsoil_sun_buried, points(datetime, temp, type='l', col='red'))
text(as.POSIXct("2022-06-02 00:00:00 UTC"), max(soil$D30cm) * 0.95,
...
tair_shad_0cm <- data.frame(datetime = round(as.POSIXct(strptime(tair_shad_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),temp = tair_shad_0cm[,3])
year(tair_shad_0cm$datetime) <- 2022

# rock shadow
tloc_shad_0cm <- read.csv(file = 'MS003T.csv', head=T)
tloc_shad_0cm <- data.frame(datetime = round(as.POSIXct(strptime(tloc_shad_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),temp = tloc_shad_0cm[,3])
year(tloc_shad_0cm$datetime) <- 2022

# soil shadow
tsoil_shad_0cm <- read.csv(file = 'MS001T.csv', head=T)
tsoil_shad_0cm <- data.frame(datetime = round(as.POSIXct(strptime(tsoil_shad_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),temp = tsoil_shad_0cm[,3])
...
with(shadmet, plot(micro$dates, TAREF, type='l', 
...
hloc_sun_0cm <- data.frame(datetime = round(as.POSIXct(strptime(hloc_sun_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),hum = hloc_sun_0cm[,3])
year(hloc_sun_0cm$datetime) <- 2022

# humidity soil sun
hsoil_sun_0cm <- read.csv(file = 'MS002H.csv', head=T)
hsoil_sun_0cm <- data.frame(datetime = round(as.POSIXct(strptime(hsoil_sun_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),hum = hsoil_sun_0cm[,3])
year(hsoil_sun_0cm$datetime) <- 2022
hsoil_sun_0cm$hum <- hsoil_sun_0cm$hum/100

# humidity soil sun buried (? cm)
hsoil_sun_buried <- read.csv(file = 'MS005H.csv', head=T)
hsoil_sun_buried <- data.frame(datetime = round(as.POSIXct(strptime(hsoil_sun_buried[,1], "%d/%m/%Y %H:%M")),"hours"),hum = hsoil_sun_buried[,3])
year(hsoil_sun_buried$datetime) <- 2022
hsoil_sun_buried$hum <- hsoil_sun_buried$hum/100
...
with(humid, plot(micro$dates, RH0cm, type='l', 
...
with(humid, plot(micro$dates, RH30cm, type='l', 
                xlim=obs_range, ylim = c(0, 1),
                xlab='', ylab='RH soil sun 30 cm (ºC)'))
with(hsoil_sun_buried, points(datetime, hum, type='l', col='red'))
text(as.POSIXct("2022-06-07 00:00:00 UTC"), max(humid$RH30cm) * 0.8,
...
hair_shad_0cm <- data.frame(datetime = round(as.POSIXct(strptime(hair_shad_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),hum = hair_shad_0cm[,3])
year(hair_shad_0cm$datetime) <- 2022

# rock shadow
hloc_shad_0cm <- read.csv(file = 'MS003H.csv', head=T)
hloc_shad_0cm <- data.frame(datetime = round(as.POSIXct(strptime(hloc_shad_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),hum = hloc_shad_0cm[,3])
year(hloc_shad_0cm$datetime) <- 2022

# soil shadow
hsoil_shad_0cm <- read.csv(file = 'MS001H.csv', head=T)
hsoil_shad_0cm <- data.frame(datetime = round(as.POSIXct(strptime(hsoil_shad_0cm[,1], "%d/%m/%Y %H:%M")),"hours"),hum = hsoil_shad_0cm[,3])
