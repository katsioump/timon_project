interp_micro <- function(metout, soil, shadmet, shadsoil, tl_m){
  ...
  for (i in 1:nrow(metout)) {
    if (j > nrow(metout_new)) break  # stop before overshooting
    for (z in 1:ncol(metout)) {
  ...
  for (z in colnames(metout_new)) {
    if (is.numeric(metout_new[[z]])) {
      metout_new[, z] <- na.approx(metout_new[, z], na.rm = FALSE, rule = 2)
    }
  }
  ...
  for (i in 1:nrow(soil)) {
    if (j > nrow(soil_new)) break  # prevent silent row auto-expansion
  ...
  for (z in colnames(soil_new)) {
    if (is.numeric(soil_new[[z]])) {
      soil_new[, z] <- na.approx(soil_new[, z], na.rm = FALSE, rule = 2)
    }
  }
  ...
  for (i in 1:nrow(shadmet)) {
    if (j > nrow(shadmet_new)) break  # prevent silent row auto-expansion
  ...
  for (z in colnames(shadmet_new)) {
    if (is.numeric(shadmet_new[[z]])) {
      shadmet_new[, z] <- na.approx(shadmet_new[, z], na.rm = FALSE, rule = 2)
    }
  }
  ...
  for (z in colnames(shadsoil_new)) {
    if (is.numeric(shadsoil_new[[z]])) {
      shadsoil_new[, z] <- na.approx(shadsoil_new[, z], na.rm = FALSE, rule = 2)
    }
  }
