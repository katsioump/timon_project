...
for (i in c(5, 6, 7, 8, 9, 12, 13, 14, 15, 16, 17, 18, 21, 22, 23, 24, 25)){
  ...
  tl_c <- subset(tl, select = c("code", "Tb", "VeDBA", "weight"))
  tl_c$Force <- tl_c$VeDBA * tl_c$weight
  tl_c <- subset(tl_c, select = -c(3, 4))
  tl_c <- tl_c[, c("code", "Tb", "Force")]
  ...
  ggplot(d_preds, aes(temp, rate)) +
    ...
    labs(x = 'Body temperature (ºC)',
         y = 'Force (mass * acceleration) (N)',
         title = paste0('Fits of selected models for TLCRP0', s)) +
    geom_text(data = data, aes(label=AIC), x = 50/100*(max(tl$Tb)), y = 98/100*(max(tl_c$rate)),
              size = 5) +
    theme(text=element_text(size=20)) +
    geom_hline(aes(yintercept = 0), linetype = 2)
  
  
  ggsave(paste0("C:/Users/Katerina/Documents/Master thesis/some graphs/TPC models_force/TLCRP0", s, ".png"), width = 20, height = 10.4)
  ...
  tl_param <- data.frame(code = params$id, weight = tl[1:8, 'weight'], model = params$model_name, AIC = d_ic$AIC, topt = params$topt, rmax = params$rmax, breadth = params$breadth)
  tl_all_param <- rbind(tl_all_param, tl_param)
}

write.csv(tl_all_param, file = "C:\\Users\\Katerina\\Documents\\Master thesis\\results\\TPC_params_force.csv")
