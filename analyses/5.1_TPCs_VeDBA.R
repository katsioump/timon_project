...
for (i in c(5, 6, 7, 8, 9, 12, 13, 14, 15, 16, 17, 18, 21, 22, 23, 24, 25)){
  ...
  ##### 8 models: 4 from Angiletta paper and 4 that look nice from graphs #####
 ...
 # plot
  ...
  ggsave(paste0("C:/Users/Katerina/Documents/Master thesis/some graphs/TPC models/TLCRP0", s, ".png"), width = 20, height = 10.4)
  ...
  tl_param <- data.frame(code = params$id, model = params$model_name, AIC = d_ic$AIC, topt = params$topt, rmax = params$rmax, breadth = params$breadth)
  tl_all_param <- rbind(tl_all_param, tl_param)
}

write.csv(tl_all_param, file = "C:\\Users\\Katerina\\Documents\\Master thesis\\results\\TPC_params_VeDBA.csv")
