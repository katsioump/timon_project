...
ggsave("results/figures/figureTPCs_tradeoff.tiff", plot = p_total,
       width = 8, height = 3 * 6, units = "cm", dpi = 600, compression = "lzw")

p3 <- ggplot(tl_mod, aes(x=breadth, y=rmax)) +
  geom_point(size = 1, shape = 16) +    # Use hollow circles
  geom_smooth(method=lm, color = "black", linewidth = 0.5) +
  labs(x = 'Thermal breadth (ºC)',
       y = expression(paste('Max. performance (', m/s^{2}, ')'))) +
  theme_classic(base_size = 10) +
