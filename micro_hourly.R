micro_hourly <- function(lat, long, elev, weather, tl){
  
  Latitude <- lat # 41.1073 
  Longitude <- long # -8.5898
  Elevation <- elev # elevation of the astronomical observatory in m
  ...
  TAIRhr <- weather$Temperature <- na.approx(weather$AirTemp) # <- na.approx(5/9 * (weather$Temperature - 32)) # convert ºF to ºC
